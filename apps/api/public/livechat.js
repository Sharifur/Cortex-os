...
`,Me=[{name:"Smileys",emojis:["😀","😃","😄","😁","😆","😅","🤣","😂","🙂","🙃","😉","😊","😇","🥰","😍","🤩","😘","😗","😚","😙","😋","😛","😜","🤪","😝","🤑","🤗","🤭","🤫","🤔","😐","😑","😶","🙄","😏","😒","😞","😔","😟","😕","🙁","☹️","😣","😖","😫","😩","🥺","😢","😭","😤","😠","😡","🤬","🤯","😳","🥵","🥶","😱","😨","😰","😥","😓","🤗","🤔","🤐","🤨","😐","😑","😶","😏","😴","🤤","😪","😵","🤐","🥴","🤢","🤮","🤧","😷"]},{name:"Hearts",emojis:["❤️","🧡","💛","💚","💙","💜","🖤","🤍","🤎","💔","❣️","💕","💞","💓","💗","💖","💘","💝","💟"]},{name:"Hands",emojis:["👍","👎","👌","✌️","🤞","🤟","🤘","🤙","👈","👉","👆","🖕","👇","☝️","👋","🤚","🖐️","✋","🖖","👏","🙌","🤝","🙏","✍️","💪","🦾"]},{name:"Objects",emojis:["🔥","✨","🎉","🎊","🎁","🏆","🥇","⭐","🌟","💫","💥","💯","✅","❌","⚠️","❓","❗","💡","📌","📎","🔗","🔒","🔑","⏰","⏳","📅","📆","🗓️","📊","📈"]},{name:"Travel",emojis:["🚀","✈️","🚗","🚕","🚙","🚌","🏠","🏢","🏥","🏦","🏪","🏫","⛺","🌍","🌎","🌏","🗺️","🏖️","🏔️","🌋"]}],Dn=[[":)","🙂"],[":-)","🙂"],[":D","😄"],[":-D","😄"],["xD","😆"],["XD","😆"],[":P","😛"],[":p","😋"],[":-P","😛"],[":'(","😢"],[":(","🙁"],[":-(","🙁"],[";)","😉"],[";-)","😉"],[":O","😮"],[":o","😮"],[":-O","😮"],[":oO","😳"],[":|","😐"],[":-|","😐"],[":/","😕"],[":-/","😕"],["<3","❤️"],["</3","💔"],[":*","😘"],["B)","😎"]];function Pn(n){let e=n;for(const[t,s]of Dn){const i=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),r=new RegExp(`(^|\\s)${i}(?=\\s|$|[.,!?])`,"g");e=e.replace(r,`$1${s}`)}return e}const jn="https://gist.githubusercontent.com/Sharifur/b40c7b54b97d43f353f1382e51c70535/raw/f6446fa378bf266cacf604f1e97f8f318e01e157/temporary-email-address-domain-list.json",ht="livechat_disposable_domains",pt="livechat_disposable_domains_ts",zn=1440*60*1e3;let Y=null;async function ut(){if(Y)return Y;try{const n=localStorage.getItem(pt),e=localStorage.getItem(ht),t=n?Number(n):0;if(e&&t&&Date.now()-t<zn){const s=JSON.parse(e);return Y=new Set(s.map(i=>i.toLowerCase())),Y}}catch(n){}try{const n=new AbortController,e=setTimeout(()=>n.abort(),4e3),t=await fetch(jn,{signal:n.signal});if(clearTimeout(e),t.ok){const s=await t.json(),r=(Array.isArray(s)?s:[]).map(o=>String(o).trim().toLowerCase()).filter(Boolean);Y=new Set(r);try{localStorage.setItem(ht,JSON.stringify(r)),localStorage.setItem(pt,String(Date.now()))}catch(o){}return Y}}catch(n){}return Y=new Set(["mailinator.com","guerrillamail.com","10minutemail.com","tempmail.com","temp-mail.org","yopmail.com","trashmail.com","fakeinbox.com","throwawaymail.com","getairmail.com","sharklasers.com"]),Y}async function Un(n){const e=n.lastIndexOf("@");if(e<0)return!1;const t=n.slice(e+1).trim().toLowerCase();return t?(await ut()).has(t):!1}function Hn(){ut()}const Fn={siteKey:"",botName:"Hi there",botSubtitle:"We typically reply in a few seconds.",welcomeMessage:null,brandColor:"#2563eb",position:"bottom-right"},De="livechat_messages_cache",Pe="livechat_session_id",je="livechat_identify_dismissed",ye="livechat_identify_name",ze="livechat_identify_email",ft="livechat_send_log",be="livechat_proactive_seen",Kn=30,Vn=6e4,Yn=3;function Wn(n,e=Fn){var q,m;const t=document.createElement("div");t.id="livechat-widget-root",t.style.cssText="position: fixed; bottom: 40px; right: 40px; z-index: 2147483646;",document.body.appendChild(t);const s=t.attachShadow({mode:"open"}),i=(q=is(e.brandColor))!=null?q:"#2563eb",r=vt(i,.35),o=vt(i,.45);t.style.setProperty("--lc-brand",i),t.style.setProperty("--lc-brand-shadow",r),t.style.setProperty("--lc-brand-shadow-hover",o),e.position==="bottom-left"&&t.classList.add("lc-position-left");const l=document.createElement("style");l.textContent=Mn,s.appendChild(l);const a={open:!1,sessionId:ts(),messages:ss(),socket:null,panel:null,askedForEmail:!1,askedForName:!1,knownName:Qn(),unread:0,sessionClosed:!1,feedbackAsked:!1,operators:(m=e.operators)!=null?m:[],host:t,cfg:n},b=document.createElement("button");b.className="lc-bubble",b.innerHTML=ps(),s.appendChild(b);const g=document.createElement("span");g.className="lc-unread",g.style.display="none",b.appendChild(g);const w=document.createElement("div");if(w.className="lc-proactive",w.style.display="none",e.welcomeMessage){w.innerHTML=`
      <button class="lc-proactive-close" aria-label="Dismiss">&#x2715;</button>
      <div class="lc-proactive-text">${T(e.welcomeMessage)}</div>
    `,s.appendChild(w);let v=!1;try{v=!!sessionStorage.getItem(be)}catch(W){}v||setTimeout(()=>{a.open||(w.style.display="block")},1500),w.querySelector(".lc-proactive-close").addEventListener("click",W=>{W.stopPropagation(),w.style.display="none";try{sessionStorage.setItem(be,"1")}catch(oe){}}),w.querySelector(".lc-proactive-text").addEventListener("click",()=>{w.style.display="none";try{sessionStorage.setItem(be,"1")}catch(W){}b.click()})}a.messages.length===0&&e.welcomeMessage&&(a.messages.push({id:"welcome",role:"agent",content:e.welcomeMessage,createdAt:new Date().toISOString()}),Z(a.messages));const p=Jn(s,n,a,j,e);p.style.display="none",a.panel=p,p._state=a,p._cfg=n;const I=()=>window.innerWidth<=480,N="position: fixed; bottom: 40px; right: 40px; z-index: 2147483646;";function h(){const v=window.visualViewport;v?t.style.cssText=`position: fixed; top: ${v.offsetTop}px; left: ${v.offsetLeft}px; width: ${v.width}px; height: ${v.height}px; z-index: 2147483646;`:t.style.cssText="position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 2147483646;"}let B=!1;function H(){if(B||!window.visualViewport)return;B=!0;const v=()=>{a.open&&(I()?h():t.style.cssText=N)};window.visualViewport.addEventListener("resize",v),window.visualViewport.addEventListener("scroll",v)}b.addEventListener("click",()=>{w.style.display="none";try{sessionStorage.setItem(be,"1")}catch(v){}if(a.open=!a.open,a.open){I()&&(h(),H()),p.classList.remove("lc-panel--closing"),p.style.display="flex",a.unread=0,g.style.display="none";const v=p.querySelector("textarea");v==null||v.focus(),gt(p)}else p.classList.add("lc-panel--closing"),setTimeout(()=>{a.open||(p.style.display="none",I()&&(t.style.cssText=N)),p.classList.remove("lc-panel--closing")},180)}),a.sessionId&&mt(n,a,j,e),Hn();function j(){Xn(p,a),!a.open&&a.unread>0?(g.textContent=String(Math.min(a.unread,99)),g.style.display="flex"):g.style.display="none"}j()}function Jn(n,e,t,s,i){var Nt;const r=document.createElement("div");r.className="lc-panel",r.innerHTML=`
    <div class="lc-header">
  ...
  `,n.appendChild(r);const o="position: fixed; bottom: 40px; right: 40px; z-index: 2147483646;";r.querySelector(".lc-newchat-btn").addEventListener("click",()=>{confirm("Start a new conversation? The current chat will be cleared.")&&we()}),r.querySelector(".lc-close").addEventListener("click",()=>{t.open=!1,r.classList.add("lc-panel--closing"),setTimeout(()=>{t.open||(r.style.display="none",window.innerWidth<=480&&(t.host.style.cssText=o)),r.classList.remove("lc-panel--closing")},180)});const b=r.querySelector(".lc-menu-btn"),g=r.querySelector(".lc-menu"),w=()=>{g.style.display="none"};b.addEventListener("click",c=>{c.stopPropagation(),g.style.display=g.style.display==="none"?"block":"none"}),r.addEventListener("click",c=>{!g.contains(c.target)&&c.target!==b&&w()}),g.addEventListener("click",async c=>{const d=c.target.closest(".lc-menu-item");if(!d)return;w();const u=d.getAttribute("data-action");if(u==="new"){if(!confirm("Start a new conversation? The current chat will be cleared."))return;we()}else if(u==="close"){if(!confirm("End this chat? You can always start a new one."))return;const _=t.sessionId;if(_)try{await fetch(`${e.apiBase}/livechat/session/${encodeURIComponent(_)}/close`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({siteKey:e.siteKey,visitorId:e.visitorId}),credentials:"omit"})}catch(f){}we(),t.messages=[{id:`system-${Date.now()}`,role:"system",content:"Chat ended. Type a message to start a new conversation.",createdAt:new Date().toISOString()}],Z(t.messages),s()}});const p=r.querySelector(".lc-messages"),I=r.querySelector(".lc-scroll-btn");p.addEventListener("scroll",()=>{const c=p.scrollHeight-p.scrollTop-p.clientHeight;I.style.display=c>120?"flex":"none"}),I.addEventListener("click",()=>{p.scrollTop=p.scrollHeight,I.style.display="none"});const N=r.querySelector(".lc-composer"),h=r.querySelector("textarea"),B=r.querySelector(".lc-hp"),H=r.querySelector('.lc-composer button[type="submit"]'),j=r.querySelector(".lc-attach-btn"),q=r.querySelector(".lc-file-input"),m=r.querySelector(".lc-pending"),v=r.querySelector(".lc-quick-replies"),W=r.querySelector(".lc-session-end"),oe=r.querySelector(".lc-session-end-btn"),ae=r.querySelector(".lc-emoji-btn"),ve=r.querySelector(".lc-emoji-pop"),Ot=r.querySelector(".lc-emoji-tabs"),Lt=r.querySelector(".lc-emoji-grid");function Ls(c){var f,y;const d=(f=h.selectionStart)!=null?f:h.value.length,u=(y=h.selectionEnd)!=null?y:d;h.value=h.value.slice(0,d)+c+h.value.slice(u);const _=d+c.length;h.setSelectionRange(_,_),h.focus()}function Rs(c){const d=Me[c];d&&(Lt.innerHTML=d.emojis.map(u=>`<button type="button" class="lc-emoji-pick" data-emoji="${u}">${u}</button>`).join(""))}ae.addEventListener("click",c=>{c.stopPropagation(),ve.style.display=ve.style.display==="none"?"block":"none"}),r.addEventListener("click",c=>{c.target instanceof Node&&!ve.contains(c.target)&&c.target!==ae&&(ve.style.display="none")}),Ot.addEventListener("click",c=>{var u;const d=c.target.closest(".lc-emoji-tab");d&&(Ot.querySelectorAll(".lc-emoji-tab").forEach(_=>_.classList.remove("lc-emoji-tab-active")),d.classList.add("lc-emoji-tab-active"),Rs(Number((u=d.getAttribute("data-cat"))!=null?u:0)))}),Lt.addEventListener("click",c=>{var u;const d=c.target.closest(".lc-emoji-pick");d&&Ls((u=d.getAttribute("data-emoji"))!=null?u:"")}),h.addEventListener("input",()=>{var u;const c=h.value,d=Pn(c);if(d!==c){const _=d.length-c.length,f=((u=h.selectionStart)!=null?u:c.length)+_;h.value=d,h.setSelectionRange(f,f)}});function we(){var c;(c=t.socket)==null||c.disconnect(),t.socket=null,t.sessionId=null,t.sessionClosed=!1,t.messages=[],t.askedForEmail=!1,t.unread=0;try{localStorage.removeItem(Pe)}catch(d){}try{localStorage.removeItem(De)}catch(d){}try{localStorage.removeItem(je)}catch(d){}W.style.display="none",h.disabled=!1,H.disabled=!1,j.disabled=!1,i!=null&&i.welcomeMessage&&(t.messages.push({id:"welcome",role:"agent",content:i.welcomeMessage,createdAt:new Date().toISOString()}),Z(t.messages)),s()}oe.addEventListener("click",we);const A=[],Bs=Date.now();let _e=!1;h.addEventListener("keydown",()=>{_e=!0}),h.addEventListener("input",()=>{_e=!0});function Rt(c){h.value=c,_e=!0,N.requestSubmit()}r._submitFromChip=Rt;const Bt=()=>{var _;const c=t.messages.some(f=>f.role==="visitor"),d=/\b(talk|speak|connect|chat)\b.*\b(human|agent|person|representative|support team)\b|\b(human|live agent|real person)\b/i,u=((_=i.welcomeQuickReplies)!=null?_:[]).filter(Boolean).filter(f=>!d.test(f));if(c||u.length===0){v.style.display="none",v.innerHTML="";return}v.style.display="flex",v.innerHTML=u.map((f,y)=>`<button data-i="${y}" type="button">${T(f)}</button>`).join(""),v.querySelectorAll("button").forEach(f=>{f.addEventListener("click",()=>{const y=Number(f.dataset.i),k=u[y];k&&Rt(k)})})};j.addEventListener("click",()=>q.click()),q.addEventListener("change",async()=>{var _;const c=(_=q.files)==null?void 0:_[0];if(q.value="",!c)return;if(c.size>10*1024*1024){P(r,`File too large: ${c.name} (max 10 MB)`);return}if(A.length>=5){P(r,"You can attach up to 5 files per message.");return}if(!t.sessionId){P(r,"Send a message first, then attach files.");return}const d=c.type.startsWith("image/")?URL.createObjectURL(c):void 0,u={id:"pending-"+Date.now(),mimeType:c.type,sizeBytes:c.size,originalFilename:c.name,url:"",localUrl:d};A.push(u),J();try{const f=await O(e,t.sessionId,c),y=A.indexOf(u);y>=0&&(A[y]=te(G({},f),{localUrl:d})),J()}catch(f){const y=A.indexOf(u);y>=0&&A.splice(y,1),d&&URL.revokeObjectURL(d),P(r,`Upload failed: ${f.message}`),J()}});function J(){if(!A.length){m.style.display="none",m.innerHTML="";return}m.style.display="flex",m.innerHTML=A.map((c,d)=>{var S;const u=c.id.startsWith("pending-"),_=(S=c.localUrl)!=null?S:"",y=c.mimeType.startsWith("image/")&&_?`<img class="lc-chip-thumb" src="${T(_)}" alt="">`:"",k=u?`${y}<span class="lc-chip-label lc-chip-uploading">Uploading…</span>`:`${y}<span class="lc-chip-label">${T(c.originalFilename)}</span><button data-i="${d}" aria-label="Remove">×</button>`;return`<span class="lc-chip${u?" lc-chip--busy":""}">${k}</span>`}).join(""),m.querySelectorAll("button[data-i]").forEach(c=>{c.addEventListener("click",()=>{const d=Number(c.dataset.i),u=A.splice(d,1)[0];u!=null&&u.localUrl&&URL.revokeObjectURL(u.localUrl),J()})})}let Fe=null,Ct=!1;const ce=c=>{var d;Ct!==c&&(Ct=c,(d=t.socket)==null||d.emit("livechat:typing",{on:c}))};h.addEventListener("input",()=>{h.style.height="auto",h.style.height=Math.min(120,h.scrollHeight)+"px",h.value.trim()?(ce(!0),Fe&&clearTimeout(Fe),Fe=setTimeout(()=>ce(!1),1500)):ce(!1)}),h.addEventListener("blur",()=>ce(!1)),h.addEventListener("keydown",c=>{c.key==="Enter"&&!c.shiftKey&&(c.preventDefault(),N.requestSubmit())}),h.addEventListener("paste",async c=>{var _;const d=(_=c.clipboardData)==null?void 0:_.items;if(!d)return;const u=[];for(const f of d)if(f.kind==="file"&&f.type.startsWith("image/")){const y=f.getAsFile();y&&u.push(y)}if(u.length){if(c.preventDefault(),!t.sessionId){P(r,"Send a message first, then paste images.");return}for(const f of u){if(f.size>10*1024*1024){P(r,`Pasted image too large: ${f.name||"image"} (max 10 MB)`);continue}if(A.length>=5)break;const y=f.name?f:new File([f],`pasted-${Date.now()}.png`,{type:f.type}),k=URL.createObjectURL(y),S={id:"pending-"+Math.random().toString(36).slice(2),mimeType:f.type,sizeBytes:f.size,originalFilename:y.name,url:"",localUrl:k};A.push(S),J();try{const F=await O(e,t.sessionId,y),R=A.indexOf(S);R>=0&&(A[R]=te(G({},F),{localUrl:k})),J()}catch(F){const R=A.indexOf(S);R>=0&&A.splice(R,1),URL.revokeObjectURL(k),P(r,`Upload failed: ${F.message}`),J()}}}}),N.addEventListener("submit",async c=>{var f;if(c.preventDefault(),B.value)return;if(t.sessionClosed){P(r,"This conversation has ended. Start a new chat below.");return}const d=h.value.trim(),u=A.some(y=>y.id.startsWith("pending-")),_=A.filter(y=>y.url&&!y.id.startsWith("pending-"));if(u&&!d){P(r,"Your file is still uploading — please wait or add a message.");return}if(!(!d&&!_.length)){if(!es()){P(r,"Slow down — too many messages in the last minute.");return}H.disabled=!0,h.value="",h.style.height="auto",ce(!1),Zn(t,d,_),A.length=0,J(),Bt(),s(),yt(r);try{const y=await zt(e,d,_.map(k=>k.id),{hp:B.value||void 0,elapsedMs:Date.now()-Bs,hadInteraction:_e});if(ie(r),t.sessionId=y.sessionId,ns(y.sessionId),"content"in y.agent&&y.agent.content){const k=(f=y.agent.id)!=null?f:"";t.messages.some(S=>S.id===k&&k)||bt(t,y.agent.content,k)}t.socket||mt(e,t,s,i),Gn(r,t,s)}catch(y){ie(r),P(r,"Could not send — please try again.")}H.disabled=!1,s()}});const $t=r.querySelector(".lc-messages");return $t.addEventListener("click",async c=>{var f,y;const d=c.target,u=d.closest(".lc-inline-skip");if(u){const k=u.getAttribute("data-step");if(k==="name")try{localStorage.setItem(ye,"skipped")}catch(S){}else if(k==="email")try{localStorage.setItem(ze,"skipped")}catch(S){}t.messages=t.messages.filter(S=>S.id!==`identify-${k}`),s();return}const _=d.closest(".lc-inline-save");if(_){const k=_.getAttribute("data-step"),S=_.closest(".lc-inline-identify"),F=S==null?void 0:S.querySelector("input"),R=(y=(f=F==null?void 0:F.value)==null?void 0:f.trim())!=null?y:"";if(k==="name"){if(!R)return;try{await Ve(e,{name:R}),t.knownName=R;try{localStorage.setItem(ye,R)}catch(X){}const ee=t.messages.findIndex(X=>X.id==="identify-name");ee>=0&&(t.messages[ee]={id:"identify-name-done",role:"system",content:`Nice to meet you, ${R}!`,createdAt:new Date().toISOString()}),s()}catch(ee){}}else if(k==="email"){const ee=X=>{var qt;F==null||F.classList.add("lc-inline-input--invalid");let K=S==null?void 0:S.querySelector(".lc-inline-error");!K&&S&&(K=document.createElement("div"),K.className="lc-inline-error",(qt=S.querySelector(".lc-inline-row"))==null||qt.after(K)),K&&(K.textContent=X)};if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(R)){ee("That doesn't look right — double-check?");return}if(await Un(R)){ee("Please use a permanent email — we can’t follow up on temporary inboxes.");return}try{await Ve(e,{email:R});try{localStorage.setItem(ze,"saved")}catch(K){}try{localStorage.setItem(je,"saved")}catch(K){}const X=t.messages.findIndex(K=>K.id==="identify-email");X>=0&&(t.messages[X]={id:"identify-email-done",role:"system",content:`Great — we'll reach out at ${R} if we miss you here.`,createdAt:new Date().toISOString()}),s()}catch(X){}}}}),$t.addEventListener("keydown",c=>{const d=c;if(d.key!=="Enter")return;const u=d.target;if(!u.matches(".lc-inline-identify input"))return;d.preventDefault();const _=u.closest(".lc-inline-identify"),f=_==null?void 0:_.querySelector(".lc-inline-save");f==null||f.click()}),Bt(),r}function mt(n,e,t,s){!e.sessionId||e.socket||(e.socket=qn(n,e.sessionId,i=>{var a,b,g,w,p,I,N,h,B,H,j,q;if(i.type==="typing"){const m=e.panel;if(!m)return;i.on?yt(m):ie(m);return}if(i.type==="session_status"&&i.status==="closed"){(a=e.socket)==null||a.disconnect(),e.socket=null,e.sessionClosed=!0;const m=e.panel;if(m){const v=m.querySelector(".lc-session-end"),W=m.querySelector("textarea"),oe=m.querySelector('.lc-composer button[type="submit"]'),ae=m.querySelector(".lc-attach-btn");v&&(v.style.display="flex"),W&&(W.disabled=!0),oe&&(oe.disabled=!0),ae&&(ae.disabled=!0),e.feedbackAsked||(e.feedbackAsked=!0,e.messages.push({id:`feedback-${Date.now()}`,role:"system",content:"__feedback__",createdAt:new Date().toISOString()}))}t();return}if(i.type==="agent_stream_start"&&i.draftId){const m=e.panel;m&&ie(m),e.messages.some(v=>v.id===i.draftId)||(e.messages.push({id:i.draftId,role:"agent",content:"",createdAt:(b=i.createdAt)!=null?b:new Date().toISOString()}),t());return}if(i.type==="agent_stream_delta"&&i.draftId&&i.delta){const m=e.messages.findIndex(v=>v.id===i.draftId);m>=0&&(e.messages[m]=te(G({},e.messages[m]),{content:e.messages[m].content+i.delta}),t());return}if(i.type==="agent_stream_end"&&i.draftId&&i.messageId){const m=e.messages.findIndex(v=>v.id===i.draftId);m>=0&&(e.messages[m]=te(G({},e.messages[m]),{id:i.messageId,content:(g=i.content)!=null?g:e.messages[m].content}),Z(e.messages),e.open||(e.unread=((w=e.unread)!=null?w:0)+1,xt()),t());return}if(i.type==="agent_suggestions"&&i.messageId&&((p=i.suggestions)!=null&&p.length)){const m=e.messages.findIndex(v=>v.id===i.messageId);m>=0&&(e.messages[m]=te(G({},e.messages[m]),{suggestions:i.suggestions.slice(0,3)}),t());return}if(i.type!=="message"||!i.messageId||i.role==="visitor"||e.messages.some(m=>m.id===i.messageId))return;const r=(I=i.operatorName)!=null?I:void 0,o=(H=i.operatorAvatarUrl)!=null?H:r&&(B=(h=(N=s==null?void 0:s.operators)==null?void 0:N.find(m=>m.name===r))==null?void 0:h.avatarUrl)!=null?B:void 0;bt(e,(j=i.content)!=null?j:"",i.messageId,i.role==="operator",i.attachments,r,o);const l=e.panel;l&&ie(l),e.open||(e.unread=((q=e.unread)!=null?q:0)+1,xt()),t()}))}function Xn(n,e){const t=n.querySelector(".lc-messages");if(!t)return;if(e.messages.length===0){t.innerHTML='<div class="lc-empty">Send us a message — we will get right back to you.</div>';return}const s=(()=>{for(let i=e.messages.length-1;i>=0;i--){const r=e.messages[i];if(r.role==="agent"||r.role==="operator")return i;if(r.role==="visitor")return-1}return-1})();t.innerHTML=e.messages.map((i,r)=>{var I,N;if(i.content==="__identify_name__"||i.content==="__identify_email__"){const h=i.content==="__identify_name__",B=h?"name":"email",H=!h&&e.knownName?`<span class="lc-inline-greet">Thanks ${T(e.knownName)}! </span>`:"",j=h?"Mind if I get your name?":`${H}If we miss you here, what's the best email to follow up on?`,q=h?"Your name":"you@example.com",m=h?"text":"email",v=h?"given-name":"email";return`<div class="lc-msg-row lc-msg-row-agent">
          ...
          <div class="lc-msg-body">
            <div class="lc-msg lc-msg-agent lc-inline-identify" data-step="${B}">
              ...
              <div class="lc-inline-row">
                <input type="${m}" class="lc-inline-input" placeholder="${q}" autocomplete="${v}" />
        ...
        </div>`}const o=i.content?i.role==="visitor"?os(i.content):as(i.content):"",l=((I=i.attachments)!=null?I:[]).map(rs).join(""),a=l?`<div class="lc-attachments">${l}</div>`:"",b=ls(i.createdAt),g=b?`<div class="lc-msg-time">${b}</div>`:"",w=r===s&&i.suggestions&&i.suggestions.length?`<div class="lc-chips">${i.suggestions.map(h=>`<button class="lc-chip" data-chip="${$(h)}">${T(h)}</button>`).join("")}</div>`:"";if(i.role==="system")return i.content==="__feedback__"?`<div class="lc-msg lc-msg-system lc-feedback" data-feedback-id="${$(i.id)}">
          ...
          ${i.operatorAvatarUrl?`<img class="lc-msg-avatar lc-msg-avatar-img" src="${$(i.operatorAvatarUrl)}" alt="${T(h)}">`:`<div class="lc-msg-avatar lc-msg-avatar-op" title="${T(h)}">${T(Ue(h))}</div>`}
          <div class="lc-msg-body">
