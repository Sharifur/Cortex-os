...
export default function ApprovalsPage() {
  ...
  useEffect(() => {
    ...
    const onRemoved = (payload: { id: string }) => {
      ...
      setSelected((prev) => { const next = new Set(prev); next.delete(payload.id); return next; });
    };
  ...
  async function bulkReject() {
    ...
    try {
      await fetch('/approvals/bulk-reject', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ ids: Array.from(selected) }),
      });
      const rejected = Array.from(selected);
      rejected.forEach(removeApproval);
    } finally {
          ...
          <div className="flex items-center gap-3 mb-4 px-1">
            ...
            {someSelected && (
              <Button
                size="sm"
                variant="outline"
                className="gap-1.5 text-destructive hover:text-destructive border-destructive/30 hover:bg-destructive/5 h-7 text-xs"
                onClick={bulkReject}
                disabled={bulkRejecting}
              >
                <XCircle className="w-3.5 h-3.5" />
                {bulkRejecting ? 'Rejecting…' : `Reject ${selected.size}`}
              </Button>
            )}
          </div>
